...
pub enum Forward<M: StateMachine> {
	...
	CommandAck {
		...
		/// The log indices assigned to the appended commands by the leader, which the
		/// follower can use to track the commit progress of the log and determine
		/// when the commands have been committed to the state machine.
		assigned: IndexRange,
	},
