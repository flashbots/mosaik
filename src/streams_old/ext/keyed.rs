...
#[derive(Debug)]
pub struct KeyedDatum<D, K: Key>(pub K, pub D);
...
impl<D, K: Key> Ord for KeyedDatum<D, K> {
	fn cmp(&self, other: &Self) -> std::cmp::Ordering {
		self.key().cmp(other.key())
	}
}

impl<D, K: Key> PartialOrd for KeyedDatum<D, K> {
	fn partial_cmp(&self, other: &Self) -> Option<std::cmp::Ordering> {
		Some(self.cmp(other))
	}
}

impl<D, K: Key> PartialEq for KeyedDatum<D, K> {
	fn eq(&self, other: &Self) -> bool {
		self.key() == other.key()
	}
}
impl<D, K: Key> Eq for KeyedDatum<D, K> {}

impl<D, K: Key> Deref for KeyedDatum<D, K> {
	type Target = D;
...
impl<D, K: Key> From<KeyedDatum<D, K>> for (K, D) {
	fn from(kd: KeyedDatum<D, K>) -> Self {
		kd.split()
	}
}

pub struct Keyed<S, D, K: Key> {
	underlying: S,
	key_fn: Box<dyn Fn(&D) -> K + 'static>,
}

impl<S, D, K: Key> Keyed<S, D, K> {
	pub fn key_of(&self, datum: &D) -> K {
		(self.key_fn)(datum)
	}
}

impl<S: Stream<Item = D>, D, K: Key> Keyed<S, D, K> {
...
impl<S: Stream<Item = D> + Unpin, D, K: Key> Stream for Keyed<S, D, K> {
...
impl<C, D, K: Key> Deref for Keyed<C, D, K> {
	type Target = C;
...
impl<C, D, K: Key> DerefMut for Keyed<C, D, K> {
	fn deref_mut(&mut self) -> &mut Self::Target {
		&mut self.underlying
	}
}

impl<S: Sink<D>, D, K: Key> Keyed<S, D, K> {
