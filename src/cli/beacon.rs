...
use mosaik::*;
use iroh::SecretKey;
use mosaik::{Digest, NetworkId, PeerId, Tag,};
...
impl BeaconCommand {
  pub async fn run(&self, _: &CliOpts) -> anyhow::Result<()> {
    ...
    let secret = self.secret.clone().unwrap_or_else(|| {
    ...
    println!("  tags   : {}", if self.tags.is_empty() { 
			"none".to_string()
		} else {
			self.tags.iter().map(|t| format!("{t:?}")).join(", ")
		});
    println!("  peers  : {}", if self.peers.is_empty() { 
			"none".to_string()
		} else {
			self.peers.iter().map(|p| p.to_string()).join(", ")
		});
		println!("  ips    : {}", network.local().addr().ip_addrs().join(", "));
		println!("  relays : {}", if self.no_relay { 
			"disabled".to_string() 
		} else { 
